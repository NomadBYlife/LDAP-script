...
from utils import connect_ldap_server, get_groups_of_ex, get_ex_users, get_dn_of_mailing_lists, \
    get_unique_member_list
...
def main_check():
    ...
    ex_user_list_mailing, _ = get_ex_users(users_groups)
    for i in ex_user_list_mailing:
        if i in mailing_list:
            del_from_mailing_list(i)
    _, ex_user_list_roles = get_ex_users(users_groups)
